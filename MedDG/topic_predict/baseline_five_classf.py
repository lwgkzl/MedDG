...
class MyModel(Model):
    def __init__(self,
        ...
        self.dim = [12, 62, 4, 20, 62]  # 每一类的子类数  diease, sym, prop, check, medicine
        self.linear_list = torch.nn.ModuleList([torch.nn.Linear(self.hidden_dim, self.dim[i]) for i in range(5)])
    ...
    def forward(self, tokens, label, **args):
        ...
        five_linear_output = [linear(seq_hidden) for linear in self.linear_list]
        topic_probs = torch.sigmoid(torch.cat(five_linear_output, -1))  # 不用5个loss，直接拼起来
        # five_topic_probs = [torch.sigmoid(output) for output in five_linear_output]
        # five_loss = [F.binary_cross_entropy(five_topic_probs[i], label[:, ], )]
        topic_weight = torch.ones_like(label) + 4 * label
        loss = F.binary_cross_entropy(topic_probs, label, topic_weight.float())
        st = 0
        # loss = None
        # for i in range(5):
        #     if loss is None:
        #         loss = F.binary_cross_entropy(five_topic_probs[i], label[:, st:st+self.dim[i]].float())
        #     else:
        #         loss += F.binary_cross_entropy(five_topic_probs[i], label[:, st:st + self.dim[i]].float())
